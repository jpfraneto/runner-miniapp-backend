...
export class NotificationService {
  ...
  async handleFrameAdded(
    ...
    try {
      ...
      if (notificationDetails) {
        ...
        this.logger.log(`Notifications enabled for user ${user.id}`);
      }
  ...
  async handleFrameRemoved(fid: number): Promise<void> {
    try {
      ...
      this.logger.log(`Notifications disabled for user ${user.id}`);
    } catch (error) {
  ...
  async handleNotificationsEnabled(
    ...
    try {
      ...
      this.logger.log(`Notification settings updated for user ${user.id}`);
    } catch (error) {
    ...
    try {
      ...
      for (const user of usersWithNotifications) {
        await this.queueNotification(
          ...
          new Date(),
          `weekly_achievement_${user.id}_${weekKey}`, // Week-specific idempotency
        );
      }
