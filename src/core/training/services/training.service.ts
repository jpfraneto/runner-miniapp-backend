...
export class TrainingService {
  ...
  async getPersonalAnalytics(fid: number): Promise<any> {
    ...
    if (sessions.length === 0) {
      return {
        ...
        averagePace: '0:00/km',
        weeklyTrends: {
          runsPerWeek: { current: 0, previous: 0, change: 0 },
          avgDistance: { current: 0, previous: 0, change: 0 },
          avgPace: { current: '0:00/km', previous: '0:00/km', improvement: 0 },
    ...
    const currentPeriodSessions = sessions.filter(
      (s) => new Date(s.completedDate) >= fourWeeksAgo,
    );
    const previousPeriodSessions = sessions.filter(
      (s) =>
        new Date(s.completedDate) >= eightWeeksAgo &&
        new Date(s.completedDate) < fourWeeksAgo,
    );
  ...
  async getWeeklySummary(fid: number): Promise<any> {
    ...
    const weekSessions = await this.runningSessionRepository
      ...
      .andWhere('session.completedDate >= :weekStart', { weekStart })
      .andWhere('session.completedDate <= :now', { now: new Date() })
      .orderBy('session.completedDate', 'DESC')
      .getMany();
    ...
    const lastWeekSessions = await this.runningSessionRepository
      ...
      .where('session.fid = :fid', { fid })
      .andWhere('session.isWorkoutImage = :isWorkout', { isWorkout: true })
      .andWhere('session.completedDate >= :lastWeekStart', { lastWeekStart })
      .andWhere('session.completedDate < :weekStart', { weekStart })
      .getMany();
  ...
  async getLeaderboard(
    ...
    if (timePeriod === 'weekly') {
      ...
      whereCondition =
        'session.completedDate >= :startDate AND session.completedDate <= :endDate';
  ...
  private calculateWeeklyTrends(
    ...
    const currentAvgTime =
      currentSessions.length > 0
        ? currentSessions.reduce((sum, s) => sum + s.duration, 0) /
          currentSessions.length
        : 0;
    const currentPace = this.calculateAveragePace(
      currentAvgDistance,
      currentAvgTime,
    );
    ...
    const previousAvgTime =
      previousSessions.length > 0
        ? previousSessions.reduce((sum, s) => sum + s.duration, 0) /
          previousSessions.length
        : 0;
    const previousPace = this.calculateAveragePace(
      previousAvgDistance,
      previousAvgTime,
    );
    ...
    return {
      ...
      avgPace: {
        ...
        improvement: this.calculatePaceImprovement(currentPace, previousPace),
      },
      consistency: {
  ...
  private calculatePaceImprovement(
    currentPace: string,
    previousPace: string,
  ): number {
    const parseTime = (pace: string) => {
      const [minutes, seconds] = pace.replace('/km', '').split(':').map(Number);
      return minutes * 60 + seconds;
    };

    const currentSeconds = parseTime(currentPace);
    const previousSeconds = parseTime(previousPace);
  ...
  private calculateStreakAnalytics(sessions: any[]): any {
    ...
    const sortedSessions = sessions
      .map((s) => new Date(s.completedDate))
  ...
  private calculateMonthlyProgression(sessions: any[]): any[] {
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      const avgPace =
        data.distance > 0
          ? this.calculateAveragePace(data.distance, data.totalTime)
          : '0:00/km';
      const improvement =
        previousDistance > 0
          ? Math.round(
  ...
  private calculateUserCommunityStats(sessions: any[]): any {
    ...
    const avgPace = this.calculateAveragePace(totalDistance, totalTime);
  ...
  private calculateCommunityBenchmarks(
    ...
    return {
      ...
      avgPace: {
        user: userStats.avgPace,
        community: this.calculateAveragePace(
          parseFloat(communityStats.avgDistance),
          parseFloat(communityStats.avgDuration),
        ),
        comparison: 'similar to average',
      },
    };
  }
